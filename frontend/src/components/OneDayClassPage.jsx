...
import "./OneDayClassPage.css";

const CATEGORY_ID = "one-day-class";
  ...
  return (
    <main className="class-page">
      <div className="class-inner">
        <h1 className="class-title">One-day Class</h1>

        {loading && <p>불러오는 중입니다...</p>}
        {errorMsg && <p className="class-error">{errorMsg}</p>}
        ...
        {!loading && !errorMsg && posts.length > 0 && (
          <ul className="class-post-list">
            {posts.map((post) => {
              ...
              return (
                <li key={post.id} className="class-post-item">
                  <h2 className="class-post-title">{post.title}</h2>
                  <p className="class-post-date">
                    {new Date(post.created_at).toLocaleDateString("ko-KR")}
                  </p>

                  <div
                    className="class-post-content"
